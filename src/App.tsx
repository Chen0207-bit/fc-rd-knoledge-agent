        ...
        {!aiFocus && view === "dashboard" && (
          <>
            ...
            <section className="task-center-grid">
              ...
              <div className="panel task-panel"><div className="panel-head"><div><p className="eyebrow">QUICK START</p><h3>快速开始</h3></div></div><div className="quick-start-list"><button onClick={() => setView("discover")}><i>01</i><span><strong>搜索论文</strong><small>从技术主题发现外部研究证据</small></span><b>→</b></button><button onClick={() => { setView("library"); setTimeout(() => fileRef.current?.click(), 0); }}><i>02</i><span><strong>知识库</strong><small>查看、导入和管理研发知识资料</small></span><b>→</b></button><button onClick={() => setView("review")}><i>03</i><span><strong>审核入库</strong><small>确认论文相关性并沉淀可靠证据</small></span><b>→</b></button><button onClick={() => setView("ip")}><i>04</i><span><strong>生成知识产权文档</strong><small>把已有资料转成可审核的申报初稿</small></span><b>→</b></button></div></div>
            </section>
