...
export default function App() {
  ...
  async function renameProject(projectId = activeProjectId) {
    ...
    try {
      await api(`/api/projects/${projectId}`, { method: "POST", body: JSON.stringify({ name, description }) });
      setProjects((items) => items.map((project) => project.id === projectId ? { ...project, name, description } : project));
      notify("项目资料已更新");
    } catch (error) { notify(error instanceof Error ? error.message : "项目更新失败"); }
  }

  async function deleteProject(projectId = activeProjectId) {
    const current = projects.find((project) => project.id === projectId);
    if (!current) return;
    if (projects.length <= 1) return notify("至少保留一个研发项目");
    if (!window.confirm(`确定删除项目“${current.name}”及其关联的论文、研发文件和成果材料吗？`)) return;
    try {
      await api(`/api/projects/${current.id}`, { method: "DELETE" });
      const next = projects.find((project) => project.id !== current.id);
      setProjects((items) => items.filter((project) => project.id !== current.id));
      if (current.id === activeProjectId && next) switchProject(next.id);
      notify("项目及其研发知识闭环已删除");
    } catch (error) { notify(error instanceof Error ? error.message : "项目删除失败"); }
  }
  ...
  return (
    <div className={`app-shell preset-${uiPreset} theme-${themePreference}${sidebarCollapsed ? " sidebar-collapsed" : ""}${chatOpen ? " assistant-open" : ""}${chatOpen && chatMinimized ? " assistant-minimized" : ""}`}>
        ...
        {!aiFocus && view === "dashboard" && (
          <>
            ...
            <section className="task-center-grid">
              ...
              <div className="panel task-panel"><div className="panel-head"><div><p className="eyebrow">QUICK START</p><h3>快速开始</h3></div></div><div className="quick-start-list"><button onClick={() => setView("discover")}><i>01</i><span><strong>搜索论文</strong><small>从技术主题发现外部研究证据</small></span><b>→</b></button><button onClick={() => setView("review")}><i>02</i><span><strong>审核入库</strong><small>确认论文相关性并沉淀可靠证据</small></span><b>→</b></button><button onClick={() => { setView("library"); setAIFocus(false); }}><i>03</i><span><strong>知识库</strong><small>查看、导入和管理研发知识资料</small></span><b>→</b></button><button onClick={() => setView("ip")}><i>04</i><span><strong>生成知识产权文档</strong><small>把已有资料转成可审核的申报初稿</small></span><b>→</b></button></div></div>
            </section>
        ...
        {aiFocus && (
          <section className="agent-workspace">
            ...
            <aside className="agent-context-column">
              <div className="context-section"><div className="context-title"><span>当前项目</span><button onClick={() => void renameProject()}>编辑</button></div><strong>{activeProject?.name || "默认研发项目"}</strong><p>{activeProject?.description || "尚未填写项目说明"}</p></div>
      ...
      {chatOpen ? <aside className={`assistant-panel${chatMinimized ? " minimized" : ""}`}>
        {!chatMinimized ? <div className="assistant-panel-head"><div><p className="eyebrow">AI COPILOT</p><strong>研知 Agent</strong><small>当前页面：{pageTitle} · {activeProject?.name || "默认项目"}</small></div><div className="chat-window-actions"><button title="折叠 AI 面板" onClick={toggleChatMinimized}>›</button><button title="关闭 AI 面板" onClick={() => setChatOpen(false)}>×</button></div></div> : <button className="assistant-minimized-label" onClick={toggleChatMinimized} title="展开 AI 助手"><span>✦</span><small>AI 助手</small></button>}
        {!chatMinimized ? <>
...
function WorkflowCandidateList({ papers }: { papers: Paper[] }) {
  return <div className="workflow-candidates"><div className="workflow-candidates-head"><strong>待确认入库论文</strong><span>{papers.length} 篇</span></div>{papers.map((paper, index) => <article className="workflow-candidate" key={`${paper.source}-${paper.externalId}`}><div className="workflow-candidate-index">{String(index + 1).padStart(2, "0")}</div><div className="workflow-candidate-main"><strong>{paper.title}</strong><small>{sourceLabel(paper.source)} · {formatDate(paper.publishedAt)} · {paper.authors.slice(0, 3).join("、") || "作者未知"}</small><p>{paper.abstract || "暂无摘要，建议打开原文核验。"}</p><a href={paper.url} target="_blank" rel="noreferrer">查看原文 ↗</a></div></article>)}</div>;
}
