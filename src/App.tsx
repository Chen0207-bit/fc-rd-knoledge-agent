    ...
    <div className={`app-shell preset-${uiPreset} theme-${themePreference}${sidebarCollapsed ? " sidebar-collapsed" : ""}${chatOpen ? " assistant-open" : ""}${chatOpen && chatMinimized ? " assistant-minimized" : ""}`}>
      ...
      <main>
        <header>
          ...
          <div className="top-actions"><div className="project-switcher"><select value={activeProjectId} onChange={(event) => switchProject(Number(event.target.value))}>{projects.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select><button onClick={() => setProjectModalOpen(true)} title="新增研发项目">＋</button><button onClick={renameProject} title="重命名当前项目">重命名</button><button className="project-delete" onClick={() => void deleteProject()} title="删除当前项目">删除</button></div><button className={`ai-trigger${chatOpen ? " active" : ""}`} onClick={() => { setChatOpen((value) => !value); setChatMinimized(false); }}><span>✦</span> AI 助手</button><span className={connected ? "live-dot" : "live-dot offline"}>{connected ? "在线" : "离线"}</span><button className="avatar" onClick={() => setCodeInput(accessCode)}>FC</button></div>
        </header>
